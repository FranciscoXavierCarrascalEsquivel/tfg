...
public class HealFXUI : MonoBehaviour
{
                                       ...
                                       Vector2 anchorMin, Vector2 anchorMax, Color col)
    {
        ...
        {
            SpawnDot(canvasParent, centerPos, anchorMin, anchorMax, col);
                           ...
                           Vector2 anchorMin, Vector2 anchorMax, Color col)
    {
        ...
        dRt.anchoredPosition = center + new Vector2(Random.Range(-120f, 120f), Random.Range(-20f, 20f));

        var dImg = dGo.AddComponent<Image>();
        dImg.color = new Color(col.r, col.g, col.b, 0.9f);

        var fx = dGo.AddComponent<HealFXUI>();
        fx.StartCoroutine(fx.AnimateDot(dRt, dImg));
    }

    private IEnumerator AnimateDot(RectTransform rt, Image img)
    {
        Vector2 vel = new Vector2(Random.Range(-60f, 60f), Random.Range(120f, 280f));
        ...
        while (elapsed < life)
        {
            ...
            vel.y -= 300f * Time.deltaTime; // Gravetat

            var c = img.color;
            c.a = Mathf.Lerp(0.9f, 0f, elapsed / life);
            img.color = c;
            yield return null;
        }
